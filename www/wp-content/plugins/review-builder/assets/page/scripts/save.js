...
SGRB.prototype.init = function(){
	...
	if (SGReview.prototype.getURLParameter('edit')) {
		jQuery('<div class="updated notice notice-success is-dismissible below-h2">' +
				'<p>Review updated.</p>' +
				'<button type="button" class="notice-dismiss" onclick="jQuery(\'.notice\').remove();"></button></div>')
		.appendTo('.sgrb-top-bar h1');
	}
					...
					"Select template": function() {
						...
						if (all != tempName) {
							if (confirm('When change the template, you\'ll lose your uploaded images and texts. Continue ?')) {
								var sgrbLink = jQuery('.sgrb-link').attr('data-href'),
									cssLink = jQuery("<link id='sgrb_core/templates/"+tempName+"-css' rel='stylesheet' type='text/css' href='"+sgrbLink+"assets/core/templates/"+tempName+".css?v="+Math.random().toPrecision(10)+"'>");
								jQuery('body').find("link[id$='"+currentTemplateName+"-css']").remove();
...
SGRB.prototype.ajaxUserRate = function(){
	...
	ajaxHandler.callback = function(response){
		if (response != 0 && response != NaN) {
			...
			jQuery('.sgrb-notice span').show().text('Please, rate all proposed categories');
			jQuery('.sgrb-notice').show();
		}
	}
	ajaxHandler.run();
