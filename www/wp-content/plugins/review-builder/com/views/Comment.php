...
class SGRB_CommentView extends SGRB_Review
{
	public function __construct()
	{
		...
		$this->setColumns(array(
			'wp_sgrb_comment.name',
			'wp_sgrb_review.title AS review_title',
			'wp_sgrb_comment.title',
			'wp_sgrb_comment.comment',
			'wp_sgrb_comment.cdate',
			'wp_sgrb_comment.id',
		));
	...
	{
		$query .= ' LEFT JOIN wp_sgrb_review ON wp_sgrb_review.id='.$this->tablename.'.review_id';
		if ((int)@$_GET["id"]) {
			$query .= ' WHERE wp_sgrb_comment.review_id='.$_GET["id"];
			if (isset($_GET["app"])) {
				if ($_GET["app"] == 1) {
