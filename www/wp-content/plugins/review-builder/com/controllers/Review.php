		...
		if ($review[0]['options']['total-rate'] == 1 || SGRB_PRO_VERSION) {
			...
			foreach ($categories as $category) {
				...
				foreach ($approvedComments as $approvedComment) {
					...
					$eachRates = SGRB_Comment_RatingModel::finder()->findBySql('SELECT AVG(rate) AS average, category_id FROM `wp_sgrb_comment_rating` WHERE category_id='.$category->getId().' GROUP BY category_id');
					$ratesArray[] = $rates;
