...
<div id="sgrb-template" title="Select template">
	<?php
	global $SGRB_AVAILABLE_TEMPLATES;
	arsort($SGRB_AVAILABLE_TEMPLATES);
	foreach($SGRB_AVAILABLE_TEMPLATES as $key=>$val):
		$isChecked = ($key == @$sgrbDataArray['template']) ? ' checked' : '';
	...
	<label class="sgrb-template-label">
		<?php if($val):?>
		<input type="radio" class="sgrb-radio" name="sgrb-template-radio" value="<?php echo $key?>"<?php echo esc_attr($isChecked);?>>
		<?php endif?>
		<?php echo $proHtml; ?>
		<img  width="200px" src="<?php echo $sgrb->app_url.'assets/page/img/'.$key.'.jpeg'; ?>">
	</label>
	<?php endforeach; ?>
</div>