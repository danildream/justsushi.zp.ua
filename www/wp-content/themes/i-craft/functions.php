...
function new_excerpt_length($length) { return 500; } add_filter('excerpt_length', 'new_excerpt_length');

function removeFromHome($query) { if ($query->is_home && !(is_paged())) { $query->set('cat','-76'); } return $query; } add_filter('pre_get_posts','removeFromHome');
